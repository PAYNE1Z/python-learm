...
class FTPClient:
    ...
    def __init__(self, server_addr, server_port):
        ...
        self.interactive_info = None   # 交互界面提示信息
    ...
    def __str__(self):
        ...
        WORK_DIR: {}
        STORAGE: {}
        """.format(self.login_user_info.get('user'),
                   self.login_user_info.get('home'),
                   self.login_user_info.get('workdir'),
                   SizeFormat.size2human(self.login_user_info.get('storage')))
    ...
    def _help(args):
        ...
        var_str = '{}_usage'.format(args[1]) if len(args) == 2 else '{}_usage'.format(args[0])
    ...
    def check_args(args, min_args=None, max_args=None, need_args=None):
        ...
        if min_args:
            if len(args) < min_args:
                print(colors.yellow('最少需要{}个参数,只传入{}个'.format(min_args, len(args))))
                return False
        if max_args:
            if len(args) > max_args:
                print(colors.yellow('最多只需要{}个参数,传入了{}个'.format(max_args, len(args))))
                return False
        if need_args:
            if len(args) != need_args:
                print(colors.yellow('需要{}个参数,传入了{}个'.format(need_args, len(args))))
                return False
        return True
    ...
    def recv_data(self, data_size, save_file=None):
        ...
        """
        Progress = progress_bar(data_size)  # 创建进度条生成器对象
        next(Progress)                      # 激活生成器
        recv_size = 0
        recv_data = b''
        while recv_size < data_size:  # 已接收的大小小于结果大小就一直收
            ...
            if save_file:
                with open(save_file, 'ab') as f:
                    f.write(data)
                    Progress.send(recv_size)  # 传递当前进度，并输出进度条
            else:
                recv_data += data
        return recv_data
    ...
    def _auth(self):
        ...
        while count < 3:
            ...
            if status == 200:
                ...
                self.interactive_info = '[{}]:[{}]>>: '.format(
                        user, self.login_user_info['workdir'])
                return True
            else:
    ...
    def _cd(self, args):
        ...
        if self.check_args(args, min_args=1, max_args=2):
            ...
            if head_data.get('ret'):
                work_dir = self.re_replace(head_data.get('workdir'))
                self.interactive_info = '[{}]:[{}]>>: '.format(self.login_user_info.get('user'),  work_dir)
            else:
    ...
    def _get(self, args):
        ...
        if self.check_args(args, min_args=2, max_args=3):
            ...
            download_temp_file_path = os.path.join(save_path, '.{}.temp'.format(file_name))  # 临时文件
                ...
                if head_data.get('ret'):
                    ...
                    if sys_partition_free_size > file_size:  # 本地存储路径分区空间大于文件大小
                        ...
                        if os.path.isfile(download_temp_file_path):  # 临时文件存在,自动续传
                            print(colors.blue('检测到[{}]文件的下载纪录，自动续传'.format(file_name)))
                            if have_size < file_size:  # 检测文件没下载完才继续下载
                                print(colors.blue('开始续传:[{}], 总大小:[{}], 续传大小:[{}] ...'.format(file_name,
                                                    SizeFormat.size2human(file_size),
                                                    SizeFormat.size2human(file_size - have_size))))
                                self.recv_data(file_size - have_size, download_temp_file_path)  # 下载剩余部分
                        else:  # 文件与临时文件都不存在就直接下载
                            print(colors.blue('开始下载:[{}], 大小:[{}] ...'.format(file_name,
                                                                              SizeFormat.size2human(file_size))))
                        ...
                        if file_md5 == save_file_md5:
                            ...
                            print(colors.green('文件[{}]下载成功,下载路径:[{}]'.format(file, save_file_path)))
                        else:
                            print(colors.red('下载出错:severMD5:{}; localMD5:{}'.format(file_md5, save_file_md5)))
                    else:
                        print(colors.yellow('本地数据目录所在分区空间[{}]不足'.format(
                            SizeFormat.size2human(sys_partition_free_size))))
                else:
                    print(colors.yellow(head_data.get('msg')))
            else:
                print(colors.yellow('本地目录[{}]文件[{}]已存在,请更改下载目录重试'.format(save_path, file_name)))
    ...
    def _put(self, args):
        ...
        if self.check_args(args, min_args=2, max_args=3):
            ...
            if put_dir and re.search('^[A-Za-z]?:|^/+', put_dir):  # 是否绝对路径 eg: F:\\abc.txt /abc.txt
                file_path = file_dir  # 绝对路径就在系统上找
            else:  # 不是绝对路径就将其与默认数据目录拼接
            ...
            if os.path.exists(file_path):  # 上传文件存在
                ...
                if head_data.get('ret'):
                    ...
                    if already_size == 0:
                        print(colors.blue('开始上传[{}], 大小:[{}]'.format(file_name, SizeFormat.size2human(file_size))))
                    else:
                        print(colors.blue('检测到[{}]文件的上传纪录，自动续传'.format(file_name)))
                        print(colors.blue('开始续传[{}], 总大小:[{}], 续传大小:[{}]'.format(file_name,
                                SizeFormat.size2human(file_size), SizeFormat.size2human(file_size-already_size))))
                ...
                print(colors.yellow('{}文件不存在'.format(file_path)))
            ...
            if args[1] in args_map:
                ...
                if obj_type == 'dir':
                    choice = strip_input(colors.yellow(
                        'Waring:使用[-d]参数将删除[{}]目录下所有文件与目录，确认请按[y|Y]>>: '.format(remove_obj)))
    ...
    def run(self):
        ...
        while retry_num < settings.RETRY_CONN_NUM:
            try:
                ...
                retry_num = 0           # 连接成功，归零
                print(colors.green('成功连接到服务器:[{}]'.format(self.server_address)))

                # 登录认证
                if self._auth():
                    print(colors.blue('请输入要执行的操作[help:查看帮助信息;exit:退出]'))
                    while True:
                        cmds = strip_input(colors.blue(self.interactive_info))
                        if not cmds: continue
                        cmd_list = cmds.split()
                        cmd_str = '_{}'.format(cmd_list[0])
